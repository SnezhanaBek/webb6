...
<div class="container">
    ...
    <p>Вы вошли как <strong><?php echo htmlspecialchars($_SESSION['admin_login'] ?? 'admin', ENT_QUOTES, 'UTF-8'); ?></strong>
        <a href="admin_logout.php" class="btn btn-delete logout-link">Выйти</a>
    </p>
    ...
    <div style="overflow-x: auto;">
        <table>
            <thead>
                <tr><th>ID</th><th>Пользователь</th><th>ФИО</th><th>Телефон</th><th>Email</th><th>Дата рождения</th><th>Пол</th><th>Языки</th><th>Биография</th><th>Контракт</th><th>Действия</th></tr>
            </thead>
            <tbody>
                <?php foreach ($applications as $app): ?>
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars(substr($app['biography'], 0, 100), ENT_QUOTES, 'UTF-8'); ?>...</td>
                        <td><?php echo $app['contract_agreed'] ? '✅ Да' : '❌ Нет'; ?></td>
                        <td>
                            <a href="admin.php?edit=<?php echo $app['id']; ?>" class="btn btn-edit">✏️ Ред.</a>
                            <a href="admin.php?delete=<?php echo $app['id']; ?>" class="btn btn-delete" onclick="return confirm('Удалить заявку №<?php echo $app['id']; ?>?')">🗑️ Удалить</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
